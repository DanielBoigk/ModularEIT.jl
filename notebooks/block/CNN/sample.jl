...
function euler_maruyama_sample(
    ...
    # (only the time *value* changes), so compiling once here and reusing it
    # for the whole trajectory is both correct and, incidentally, far faster
    # than recompiling per step.
    xt_trial = xt |> dev
    t_trial = fill(T, num_samples) |> dev
    ...
    for step in 1:num_steps
        ...
        t_arr = fill(Float32(t), num_samples)
