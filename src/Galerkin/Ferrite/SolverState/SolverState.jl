...
function FerriteSolverState(fe::FerriteFESpace, σ::AbstractVector, d, ∂d, n, ∂n)
    ...
    L_fak = factorize(L)
    Σ = zeros(fe.m - 1)
    opt = FerriteOptState(nothing, nothing, 0.0, 0.0, 0.1, 0, 1e-5, LinearMap(spdiagm(ones(fe.n))), copy(δ), nothing, nothing, nothing, nothing, nothing)
...
function update_L!(state::FerriteSolverState, fe::FerriteFESpace, factorized::Bool=false, dirichlet=false)
    ...
    if factorized
        ...
        try
            state.L_fac = factorize(state.L)
            if dirichlet
                state.LD_fac = factorize(state.LD)
            end
        catch e
